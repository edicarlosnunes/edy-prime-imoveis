...
export function Proof() {
  ...
  return (
    <section id="diferenciais" data-sec="diferenciais" className="border-y border-line bg-bone">
      <div className="mx-auto max-w-[1240px] px-6 py-16 lg:px-8">
        {(data.eyebrow.trim() || data.title.trim() || data.text.trim()) && (
          <div className="reveal mb-12 max-w-2xl">
            ...
            {data.title.trim() && (
              <h2 data-t="heading" className="display mt-5 text-[calc(clamp(1.9rem,3.6vw,2.8rem)*var(--h-scale,1))] text-deep">
                <Lines text={data.title} />
              </h2>
            )}
            {data.text.trim() && (
              <p data-t="body" className="mt-4 text-sm leading-relaxed text-muted">
          ...
          {data.items.map((item, index) => {
            ...
            return (
              ...
              >
                ...
                <p data-t="card" className="display mt-4 text-2xl text-deep">{item.title}</p>
                <p data-t="body" className="mt-2 text-sm leading-relaxed text-muted">{item.text}</p>
              </div>
            );
          })}
        </div>
        ...
        {data.showDistricts && site.districts.length > 0 && (
          <div data-t="caption" className="reveal label-xs mt-14 flex flex-wrap items-center gap-x-3 gap-y-2 border-t border-line pt-8 text-muted">
