...
const propertyInput = z.object({
  ...
  /** Mantido para compatibilidade; quando omitido deriva do valor de venda/aluguel. */
  ...
  condominiumName: z.string().max(200).nullable().optional(),
  latitude: z.number().min(-90).max(90).nullable().optional(),
  longitude: z.number().min(-180).max(180).nullable().optional(),
  proximities: z.array(z.string().max(160)).max(40).nullable().optional(),
  ...
  kitchens: z.number().int().min(0).max(40).nullable().optional(),
  livingRooms: z.number().int().min(0).max(40).nullable().optional(),
  landArea: z.number().min(0).max(1_000_000).nullable().optional(),
  frontage: z.number().min(0).max(1_000_000).nullable().optional(),
  depth: z.number().min(0).max(1_000_000).nullable().optional(),
  hectares: z.number().min(0).max(1_000_000).nullable().optional(),
  floor: z.number().int().min(-10).max(300).nullable().optional(),
  totalFloors: z.number().int().min(0).max(300).nullable().optional(),
  constructionYear: z.number().int().min(1000).max(3000).nullable().optional(),
  solarPosition: z.string().max(80).nullable().optional(),
  seaDistance: z.number().min(0).max(1_000_000).nullable().optional(),
  salePrice: z.number().min(0).max(999_999_999).nullable().optional(),
  rentPrice: z.number().min(0).max(999_999_999).nullable().optional(),
  iptuPeriod: z.string().max(40).nullable().optional(),
  negotiationTerms: z.string().max(2000).nullable().optional(),
  internalNotes: z.string().max(6000).nullable().optional(),
...
function toRow(input: z.infer<typeof propertyInput>) {
  return {
    ...
    type: input.type,
    price: input.price ?? input.salePrice ?? input.rentPrice ?? 0,
    ...
    condominiumName: input.condominiumName?.trim() || null,
    latitude: input.latitude ?? null,
    longitude: input.longitude ?? null,
    proximities: input.proximities
      ? JSON.stringify(input.proximities.filter((proximity) => proximity.trim().length > 0))
      : null,
    ...
    solarPosition: input.solarPosition?.trim() || null,
    seaDistance: input.seaDistance ?? null,
    salePrice: input.salePrice ?? null,
    rentPrice: input.rentPrice ?? null,
    iptuPeriod: input.iptuPeriod?.trim() || null,
    negotiationTerms: input.negotiationTerms?.trim() || null,
