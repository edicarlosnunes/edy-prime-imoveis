...
import { useAdminCaptures, useCapture, useCreateCapture, useMarkCaptureLost, useReopenCapture, useSetCaptureDocStatus, useSetCaptureNextAction, useSetCaptureStage, useSaveCaptureAppraisal, useMarkCaptureConverted } from "../../queries/admin";
...
function Detail({id,onClose}:{id:number|null;onClose:()=>void}) {
  const q=useCapture(id); const setStage=useSetCaptureStage(); const next=useSetCaptureNextAction(); const appraisal=useSaveCaptureAppraisal(); const docs=useSetCaptureDocStatus(); const lost=useMarkCaptureLost(); const reopen=useReopenCapture(); const converted=useMarkCaptureConverted();
  ...
  function saveAppraisal(){const v=prompt("Valor estimado da avaliação:",c.estimatedPrice?String(c.estimatedPrice):""); if(v===null)return; run(appraisal.mutateAsync({id:c.id,estimatedPrice:v?Number(v.replace(/\D/g,"")):null,note:null}))}
  function markLost(){const reason=prompt("Motivo da perda:");if(reason)run(lost.mutateAsync({id:c.id,reason,detail:null}))}
  function markConverted(){const p=prompt("ID do imóvel já cadastrado para vincular a esta captação:");if(p&&Number(p)>0)run(converted.mutateAsync({id:c.id,propertyId:Number(p)}))}
  return <Modal open={id!==null} onClose={onClose} title={`Captação #${c.id}`} wide><div className="space-y-4"><div className="flex flex-wrap gap-2"><Badge tone={c.stage==="captado"?"green":c.stage==="perdido"?"red":"brass"}>{c.stage}</Badge><Badge>{c.docStatus}</Badge></div><div className="grid gap-4 md:grid-cols-2"><Card title="Proprietário"><b>{c.owner?.name}</b><div className="mt-2 text-sm text-muted">{c.owner?.phone}</div><div className="mt-3 flex gap-2"><a href={`tel:${c.owner?.phone||""}`}><Btn tone="outline"><Phone className="h-4 w-4"/>Ligar</Btn></a>{c.owner?.phone&&<a href={waLink(c.owner.phone)} target="_blank" rel="noreferrer"><Btn tone="outline"><MessageCircle className="h-4 w-4"/>WhatsApp</Btn></a>}</div></Card><Card title="Imóvel"><div>{c.propertyType||"—"} · {c.district||c.city}</div><div className="mt-1 text-xl">{money(c.askingPrice)}</div><div className="text-xs text-muted">{c.address||""}</div></Card></div><Card title="Próxima ação"><div className="text-sm">{c.nextAction||"Sem próxima ação"} · {dateTimeLabel(c.nextActionAt)}</div><Btn className="mt-3" tone="outline" onClick={nextAction}>Agendar / reagendar</Btn></Card><Card title="Avaliação"><div className="text-sm">Estimativa: {money(c.estimatedPrice)}</div><Btn className="mt-3" tone="outline" onClick={saveAppraisal}>Registrar avaliação</Btn></Card><Card title="Documentação"><Select value={c.docStatus} onChange={e=>run(docs.mutateAsync({id:c.id,docStatus:e.target.value as any}))}><option value="nao_iniciado">Não iniciado</option><option value="pendente">Pendente</option><option value="parcial">Parcial</option><option value="completo">Completo</option></Select></Card><Card title="Etapa"><div className="flex flex-wrap gap-2">{STAGES.map(([key,label])=><Btn key={key} tone={c.stage===key?"brass":"outline"} onClick={()=>run(setStage.mutateAsync({id:c.id,stage:key}))}>{label}</Btn>)}</div></Card><div className="flex flex-wrap gap-2">{c.stage==="perdido"?<Btn onClick={()=>run(reopen.mutateAsync({id:c.id}))}>Reabrir</Btn>:<Btn tone="danger" onClick={markLost}>Marcar perdido</Btn>}{!c.convertedPropertyId&&<Btn tone="brass" onClick={markConverted}>Vincular imóvel e captar</Btn>}{c.convertedPropertyId&&<Badge tone="green">Imóvel #{c.convertedPropertyId}</Badge>}</div>{c.history?.length>0&&<Card title="Histórico"><div className="space-y-2">{c.history.map(h=><div key={h.id} className="border-b border-line pb-2 text-xs"><b>{h.action}</b> · {dateTimeLabel(h.createdAt)}<div className="text-muted">{h.detail||""}</div></div>)}</div></Card>}</div></Modal>
}
